...
class EnemyPlane:
    def __init__(self, img_path, x, y, window):
        self.img = pygame.image.load(img_path)  # 敌机图片路径
        # 敌机第一次出现在窗口的位置
        self.x = x
        self.y = y
        # 敌机所在的窗口
...
def main():
    ...
    enemy_list = []
    for _ in range(105):
        enemy_plane = EnemyPlane("res/img-plane_%d.png" % random.randint(1,7),random.randint(0,WINDOW_WIDTH - 100),random.randint(-300,-68),window)
